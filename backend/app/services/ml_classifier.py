...
try:
    ...
    print(f"[WARN] PyTorch not available ({e}). Using mock classifier.")
    PYTORCH = False
...
_MODEL_PATH = _MODEL_DIR / "inflation_classifier.pkl"
_trained_model = None

if PYTORCH and _MODEL_PATH.exists():
    try:
        _trained_model = HybridCosmoClassifier()
        _trained_model.load_state_dict(torch.load(_MODEL_PATH, map_location='cpu'))
        _trained_model.eval()
        print(f"[OK] Loaded trained hybrid classifier from {_MODEL_PATH}")
    except Exception as e:
...
def classify_inflation_model(tda_result: dict) -> dict:
    ...
    if _trained_model is not None:
        # ── Real inference ──────────────────────────────────────────
        map_preview = tda_result.get("map_preview")
        if map_preview is not None:
            patch = np.array(map_preview, dtype=np.float32)
            patch_tensor = torch.tensor(patch).unsqueeze(0).unsqueeze(0)
        else:
            patch_tensor = torch.randn(1, 1, 64, 64)
            
        feat_tensor = torch.tensor(feats, dtype=torch.float32).unsqueeze(0)
        
        with torch.no_grad():
            logits = _trained_model(patch_tensor, feat_tensor)
        ...
        return {
            ...
            "note": "Prediction from trained PyTorch Hybrid CNN+TDA classifier.",
            "model_type": "PyTorchHybrid",
            "model_trained": True,
        }
    else:
        # ── Mock fallback ───────────────────────────────────────────
